...
from common import import_simplejson, urlencode_noplus
...
class StreamListener(object):
    ...
    def on_data(self, data):
        ...
        if 'in_reply_to_status_id' in data:
            ...
            if self.on_limit(json.loads(data)) is False:
                return False
    ...
    def _run(self):
        ...
        while self.running:
            ...
            try:
                ...
                self.auth.apply_auth(url, 'GET', self.headers, self.parameters)
                # conn.set_debuglevel(10)
                conn.connect()
                conn.sock.settimeout(self.timeout)
                import urllib
                self.body = urllib.urlencode({'with': 'followings'})
                conn.request('GET', self.url, body=self.body, headers=self.headers)
